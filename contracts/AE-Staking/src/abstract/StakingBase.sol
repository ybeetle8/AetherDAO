...
import {IAE} from "../interfaces/IAE.sol";
...
abstract contract StakingBase is Ownable, IStaking {
    ...
    function getAPYRate365D() internal pure virtual returns (uint256);
    ...
    function getStakePeriod365D() internal pure virtual returns (uint256);
    ...
    function getTeamThresholdTier8() internal pure virtual returns (uint256);
    function getTeamThresholdTier9() internal pure virtual returns (uint256);
    ...
    uint8 internal constant MAX_STAKE_INDEX = 4;
    ...
    uint256 internal constant TEAM_REWARD_TIER_8 = 31;
    uint256 internal constant TEAM_REWARD_TIER_9 = 35;

    // Personal Stake Thresholds for Each Tier
    ...
    uint256 internal constant AE_BUY_BURN_FEE_BPS = 50;
    ...
    string public constant name = "Staked AE";
    string public constant symbol = "sAE";
    ...
    function getTeamRewardRates()
        external
        pure
        returns (uint256[9] memory rewardRates)
    {
        ...
        rewardRates[7] = TEAM_REWARD_TIER_8;
        rewardRates[8] = TEAM_REWARD_TIER_9;
    }
        ...
        )
    {
        ...
        if (currentTier < 9) {
            IStaking.TeamTier[9] memory tiers = _getTeamTiers();
            nextTierThreshold = tiers[8 - currentTier].threshold;
            progressToNextTier =
    ...
    function _getTeamTiers()
        internal
        pure
        returns (IStaking.TeamTier[9] memory)
    {
        return [
            IStaking.TeamTier(getTeamThresholdTier9(), TEAM_REWARD_TIER_9),
            IStaking.TeamTier(getTeamThresholdTier8(), TEAM_REWARD_TIER_8),
    ...
    /// @notice Get tier based on personal stake value
    /// @param personalStake User's current stake value
    ...
    /// @notice Get tier based on team KPI
    /// @param teamKPI User's team total investment value
    /// @return tier Tier level (0-9)
    ...
    function setAE(address _ae) external onlyOwner {
        require(_ae != address(0), "AE address cannot be zero");
        AE = IAE(_ae);
        AE.approve(address(ROUTER), type(uint256).max);
        emit AEContractSet(_ae);
    }

    function emergencyWithdrawAE(
        address to,
        uint256 _amount
    ) external onlyOwner {
