...
> **重要**: 以下地址必须在部署前确定，部分地址写入合约构造函数后**不可修改**（immutable）。

| 地址用途 | 是否可修改 | 配置文件字段 | 说明 |
|----------|-----------|-------------|------|
| `marketingAddress` | 可修改 | `addresses.marketingAddress` | 营销地址，AE 合约 constructor + LiquidityStaking |
| `rootAddress` | **不可修改** | `addresses.rootAddress` | 推荐系统根节点地址 |
| `feeRecipient` | **不可修改** | `addresses.feeRecipient` | 质押手续费接收地址 |
| `buyTaxNodeRewardAddress` | **不可修改** | `addresses.buyTaxNodeRewardAddress` | 买入税节点奖励接收地址 |
| `buyTaxCommunityRewardAddress` | **不可修改** | `addresses.buyTaxCommunityRewardAddress` | 买入税社区奖励接收地址 |
| `marketingFundAddress` | **不可修改** | `addresses.marketingFundAddress` | 营销基金地址 |
| `weeklyTop15RewardAddress` | **不可修改** | `addresses.weeklyTop15RewardAddress` | 每周 Top15 奖励地址 |
| `educationFundAddress` | **不可修改** | `addresses.educationFundAddress` | 教育基金地址 |
...
步骤1: 部署 Staking 合约
         ↓
步骤2: 部署 AE 代币合约 (依赖 Staking 地址)
         ↓
步骤3: 初始化 AE 白名单
         ↓
步骤4: Staking.setAE() 关联 AE 代币
         ↓
步骤5: 创建 AE/USDC 交易对 (通过 PancakeSwap Factory)
         ↓
步骤6: AE.setPair() 设置交易对
         ↓
步骤7: 转移 20,000,000 AE → Staking 合约
         ↓
步骤8: 部署 LiquidityStaking 合约 (依赖 AE + Pair + Staking 地址)
         ↓
步骤9: AE.setLiquidityStaking() 关联
         ↓
步骤10: 部署 FundRelay 合约 (依赖 AE 地址)
         ↓
步骤11: AE.setFundRelay() 关联
         ↓
步骤12: 转移 18,740,000 AE → 节点奖励地址
         ↓
步骤13: 转移 1,260,000 AE → 跨链储备地址
         ↓
步骤14: 验证所有余额和配置
         ↓
步骤15: 合约开源验证 (BSCScan)
...
=== 脚本执行完毕 ===
         ↓
步骤16: [手动] 管理员在 PancakeSwap 添加流动性
         ↓
步骤17: [手动] AE.setPresaleActive(false) 开放交易
```
...
脚本自动调用 `hardhat verify` 在 BSCScan 上开源所有 4 个合约
...
主网部署使用专用配置文件 `ae-mainnet-config.json`，管理员需要在部署前替换所有 `【替换】` 标记的地址：
...
2. **地址核验**: 部署前务必反复核验所有地址，构造函数中的 immutable 地址部署后无法修改
3. **Gas 价格**: 脚本默认 3 Gwei，BSC 拥堵时可能需要调高
4. **分步执行**: 如果部署中途失败，需要根据已部署的合约地址调整脚本继续执行
