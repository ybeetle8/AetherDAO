/**
 * 开启预售脚本
 * 调用 AE.setPresaleActive(true) 开启预售模式，限制用户从 DEX 买入
 *
 * 用法: npx hardhat run scripts/startPresale.js --network bsc
 */
...
async function main() {
  ...
  if (currentStatus) {
    console.log("预售已经开启，无需操作。");
    return;
  }

  // 执行开启预售
  console.log("\n正在调用 setPresaleActive(true) ...");
  try {
    const tx = await ae.setPresaleActive(true);
  ...
  if (newStatus) {
    console.log("✓ 预售已开启，用户无法从 DEX 买入 AE");
  } else {
